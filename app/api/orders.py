...
# Alternative approach - manually construct response objects
from app.schemas.order import CartItemSchema, WishlistItemSchema
from app.schemas.product import ProductSchema  # You may need to import this
...
async def add_to_cart(
    ...
    # Check if variant exists (if specified) and get it
    variant_price = None
    variant = None
    if cart_item_data.variant_id:
        variant_result = await db.execute(
    ...
    # Now fetch the cart item with all relationships in a single query
    result = await db.execute(
    ...
    row = result.first()
    if row:
        cart_item_obj, product_obj, variant_obj = row
        # Manually attach the loaded relationships
        cart_item_obj.product = product_obj
        cart_item_obj.variant = variant_obj
        return cart_item_obj
    
    # Fallback - this shouldn't happen
    raise HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        detail="Failed to retrieve cart item"
    )
...
async def update_cart_item(
    ...
    cart_check_result = await db.execute(
        select(CartItem).where(
            and_(
                CartItem.id == cart_item_id,
                CartItem.user_id == current_user.id
    ...
    cart_item = cart_check_result.scalar_one_or_none()
    ...
    # Fetch with relationships using explicit join
    result = await db.execute(
        select(CartItem, Product, ProductVariant)
        .join(Product, CartItem.product_id == Product.id)
        .outerjoin(ProductVariant, CartItem.variant_id == ProductVariant.id)
    ...
    raise HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        detail="Failed to retrieve updated cart item"
    )
...
async def add_to_wishlist(
    ...
    # Now fetch the wishlist item with product in a single query
    result = await db.execute(
        select(Wishlist, Product)
        .join(Product, Wishlist.product_id == Product.id)
        .where(Wishlist.id == wishlist_item_id)
    )
    
    row = result.first()
    if row:
        wishlist_obj, product_obj = row
        # Manually attach the loaded relationship
    ...
    raise HTTPException(
        status_code=status.HTTP_500_INTERNAL_SERVER_ERROR,
        detail="Failed to retrieve wishlist item"
    )
...
# # # Fixed add_to_cart endpoint with explicit join loading
# # @router.post("/cart", response_model=CartItemSchema, status_code=status.HTTP_201_CREATED)
# # async def add_to_cart(
# #     cart_item_data: CartItemCreate,
# #     current_user: User = Depends(get_current_active_user),
...
# #         # Re-fetch with eager loading to ensure all relationships are loaded
...
# # # Fixed add_to_wishlist endpoint with explicit join loading
# # @router.post("/wishlist", response_model=WishlistItemSchema, status_code=status.HTTP_201_CREATED)
# # async def add_to_wishlist(
# #     wishlist_item_data: WishlistItemCreate,
# #     current_user: User = Depends(get_current_active_user),
